...
struct KnockoutStageView: View {
    ...
    private var knockoutTeams: Set<String> {
        // For knockout stage, we know there are exactly 4 qualified teams (top 2 from each group)
        // We can determine this from the number of groups: 2 groups × 2 teams each = 4 teams
        // This is more accurate than counting all team IDs from matches since some matches might have "TBD"
        return []  // We'll return an empty set for calculation but use hardcoded 4 in display
    }
    
    private var qualifiedTeamsCount: Int {
        // In knockout stage, we always have top 2 teams from each group
        // With 2 groups, that's exactly 4 teams
        return min(tournament.groups.count * 2, 4)
    }
